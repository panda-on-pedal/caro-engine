...
1. ~~**Pure helpers + specs**: `src/shared/results.ts`, `src/ui/tournament.ts`, both spec files → `npm test` green.~~ **Done** 2026-07-20 — `BoardSession.p1/p2` typed as the narrower `TournamentDifficulty` (not the full `Difficulty`), so `TOURNAMENT_TIME_BUDGET_MS` indexing is type-safe without a cast.
2. ~~**Server results API**: modify `server.ts` → verify with `npm start` + curl POST/GET `/api/results`; bad payload → 400; `results.json` grows.~~ **Done** 2026-07-20 — verified via curl (GET empty `[]`, POST valid echoes + persists, POST invalid → 400, unsupported method → 405 with `Allow: GET, POST`).
...
5. ~~**Layout refactor** (`index.html` + minimal `app.ts` lookups) → visual check in all modes; margin left of red line empty; buttons right of board.~~ **Done** 2026-07-20 — 3-column CSS grid (`.margin-col` / `.logo-col` / `main`), `.play-row` with the board left and a vertical `.controls` stack right; verified via Playwright (grid columns left-to-right in the expected order).
6. ~~**Multi-board tournament in `app.ts`**: sessions, tabs, board-count, rotation, per-session loops (tournament budgets, zero delay), game-end → POST → restart, Pause-all, undo hidden, persistence skipped, pool resize on count change (bump generation).~~ **Done** 2026-07-20. One departure from the sketch above: human modes did **not** get folded into `BoardSession`/`sessions[0]` as originally proposed — they kept the pre-existing global `state`/`past`/`future`/`patternStore`, and `render()`/`updateHistoryButtons()` branch on `mode` via `activeGameState()`/`activeBusy()` to pick the right data source. This avoided rewriting the working, well-tested human-mode undo/redo/persistence path for architectural uniformity alone. `pairingCounter` resets to 0 on every entry into ai-ai, including board-count changes (a count change is treated as "restart the tournament"), which the original sketch left unspecified.
7. ~~**Stats panel**: fetch + aggregate + table render.~~ **Done** 2026-07-20 — table rebuilt from `aggregateResults()` after every game-end POST and on ai-ai entry.
8. ~~**Full pass**: `npm run typecheck && npm run lint && npm test` + manual verification.~~ **Done** 2026-07-20 — typecheck/lint/new specs green; manual verification via a scripted Playwright run (see Verification section below) rather than by hand.
