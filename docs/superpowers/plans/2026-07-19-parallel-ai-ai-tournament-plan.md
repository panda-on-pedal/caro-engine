...
3. **Worker + pool + build**: new files, build script, `STATIC_FILES` → `npm run build` emits both bundles; `/engineWorker.js` served.
4. **Route existing modes through the pool** (no other behavior change): size-1 pool at init; `runAiMove` awaits `pool.requestMove`; `cancelAll()` in `resetForMode` → all 3 modes still play; UI responsive during hard thinks.
