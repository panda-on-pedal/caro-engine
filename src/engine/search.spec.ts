...
describe("threat search integration", () => {
  ...
  it("plays the fork force cell under threatSearch even at maxDepth 1", () => {
    const board = parseBoard(`
      .......
      .......
      O.XXX..
  ...
  it("expert-style config proves a multi-ply force with WIN_SCORE", () => {
    const board = parseBoard(`
      .......
      .....X.
      ...XX..
    ...
    expect(result.move).toEqual({ row: 2, col: 5 });
    expect(result.score).toBeGreaterThanOrEqual(WIN_SCORE);
    expect(result.principalVariation.length).toBeGreaterThanOrEqual(3);
  });

  it("does not play open-three extend when opponent already has a four (catalog #15)", () => {
    const board = parseBoard(`
       8  9 10 11 12 13 14
    7  .  .  .  .  .  .  .
    8  .  .  X  .  .  .  .
   ...
   12  .  .  .  .  .  .  .
    ...
    expect(result.move).not.toEqual({ row: 8, col: 12 });
    expect([
      { row: 11, col: 14 },
      { row: 11, col: 15 },
    ]).toContainEqual(result.move);
